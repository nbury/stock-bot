...
class trader:
    ...
    def updateStockSet(self,stocks):
        for stock in stocks:
            ...
            avg10 =0
            avg30 =0
            while(i >0 and i> i-60):
                p = self.portfolio[stock]['prices'][i]
                if m < 10:
                    avg5 = avg5 + p
                if m < 20:
                    avg10 = avg10 + p
            ...
            if(m != 0):
                ...
                if(m > 20):
                    ...
                    avg10 = avg10/m
                avg30 = avg30 /m   
            #print(stock+" | avg5 : "+ str(avg5)+", avg10 : "+ str(avg10))
            self.portfolio[stock]['5min_avg'] = avg5
            self.portfolio[stock]['10min_avg'] = avg10
            #print(stock+" | last:"+str(lastPrice)+", 5min:"+str(avg5)+", 10min:"+str(avg10))
            if(lastPrice !=0):
    ...
    def sell(self,symbol):
        ...
        pc = stock['5min_avg'] - stock['10min_avg']
        pc2 = stock['last_price'] - stock['10min_avg']
        pc3 = stock['10min_avg'] - stock['30min_avg']
        if(pc < default_stop and pc2 < default_stop and pc3 < default_stop ):
            return True
        return False
    ...
    def buy(self,symbol):
        stock =  self.portfolio[symbol]
        pc = stock['5min_avg'] - stock['10min_avg']
        if(stock['active'] or symbol in self.blacklist or self.portfolio['num_active']>100 or stock['last_price'] == 0 or self.sell(symbol) or pc < .2):
            return -1
    ...
    def run(self):
        ...
        while True:
            ...
            for stock in self.allStocks:
                try:
                    if (isinstance(self.portfolio[stock]['active'],bool) and self.portfolio[stock]['active'] and self.sell(stock)):
